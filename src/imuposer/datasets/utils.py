...
def get_dataset(config=None, test_only=False):
    ...
    if model in ("GlobalModelIMUPoser", "ReconIMUPoser", "StagedIMUPoser", "TransformerIMUPoser", "DiffusionIMUPoser", "AvatarPoserModel", "CNN1DIMUPoser", "CodebookIMUPoser", "ActivityIMUPoser"):
...
def get_datamodule(config):
    model = config.model
    # load the dataset
    if model in ["GlobalModelIMUPoser", "GlobalModelIMUPoserFineTuneDIP", "ReconIMUPoser", "StagedIMUPoser", "TransformerIMUPoser", "DiffusionIMUPoser", "AvatarPoserModel", "CNN1DIMUPoser", "CodebookIMUPoser", "ActivityIMUPoser"]:
...
class IMUPoserDataModule(pl.LightningDataModule):
    ...
    def test_dataloader(self):
        return DataLoader(self.test_dataset, batch_size=self.config.batch_size, collate_fn=pad_seq, num_workers=int(os.environ.get("NUM_WORKERS", "8")), shuffle=False)
